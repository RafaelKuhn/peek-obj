...
use crate::{render_string, timer::Timer, TerminalBuffer, UVec2, Vec3};
...
pub struct App {
	...
	// pub has_paused_anim: bool,

	// TODO: struct 'user polled data'
	// maybe can take a Rc<Camera>
	pub user_dir: Vec3,
	pub user_pos: Vec3,
	pub user_rot: Vec3,
...
impl App {
	...
	pub fn try_saving_screenshot(&mut self, timer: &Timer) {
		...
		let diff_since_last_ss = now - self.last_screenshot_instant;
	
		let last_one_was_too_recent = diff_since_last_ss < App::SCREENDUMP_DELAY_DURATION;
		if last_one_was_too_recent {
			let diff_secs = (App::SCREENDUMP_DELAY_MS - diff_since_last_ss.as_millis() as u64) as f32 / 1000.0;
